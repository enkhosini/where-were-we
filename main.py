...
def format_response(data):
    ...
    try:
        ...
        print('did not receive data to insert', e)
        entry_series = {
        'timestamp':                [timestamp_now()],
        'Main Topic':               [pd.NA],
        'Main Topic description':   [pd.NA],
        'Second Topic':             [pd.NA],
        'Second Topic description': [pd.NA],
        'Level':                    [pd.NA],
        }
        return 
...
def transcription():
    ...
    recorder = AudioToTextRecorder(
        ...
        realtime_processing_pause=0.5,      # increase this — fewer realtime passes = less CPU load
        device="cpu",
        beam_size=1,
        spinner=True,
    ...
    while True:
        try:
            ...
            response_formatted = format_response(response)
